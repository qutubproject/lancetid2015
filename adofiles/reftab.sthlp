...
{p 2 4}{cmd:reftab} {help varlist} [{help using}] [{help if}] [{help in}], {opth by:var(varname)} {opt ref:cat(base_value)} [{opt logit}] [{opth iv(varname)}]
{break} [{opth controls(varlist)}] [{opt cl:uster(clustvar)}] [{opt dec:imals()}] [{opt n}] [{opt se:m}] [{it:xml_tab_options}]
...
{p}If {bf:iv} is specified, the final panel will contain the second-stage estimate of the effect of the variable specified, instrumented on the full set of category indicators. It will include an adjusted estimate as well if {opt controls()} are specified.

{p}If {bf:logit} is specified, the base and adjusted regressions will report odds ratios.
