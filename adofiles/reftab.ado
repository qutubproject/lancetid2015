	...
	foreach var of varlist `anything' {
		...
		* Get means, SEs for reference category
			...
			qui tabstat `var' if `byvar' == `refcat' , s(mean `sem') save
				...
				mat theReference = nullmat(theReference) \ theNextVar
...
* Compile Results
	...
	mat colnames theReference = "`refname'"
	...
	if "`n'" == "n" {
		mat n = refN , n , J(1,colsof(results)-colsof(n)-1,.)
...
* Print to xml_tab
	...
	if "`iv'" != "" {
		...
		local n2 = 2*`nlevels' - 1
		local n3 = 3*`nlevels' - 2
		
		xml_tab results `using' , `options' cblanks(`nlevels' `n2' `n3') format((SCLB0) (SCCB0 `theFormats' NCRR0)) ///
